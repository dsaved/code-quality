    ...
    steps:
      ...
      - name: Check variable naming conventions
        run: |
          ...
          # Check const, let, var declarations - exclude PascalCase patterns for components, schemas, guards, etc.
          VARIABLE_ERRORS=$(grep -rn -E "(const|let|var)\s+[A-Z_][a-zA-Z0-9_]*\s*=" src --include="*.ts" --include="*.tsx" --include="*.js" --include="*.jsx" | \
            grep -v -E "(const|let|var)\s+[A-Z][A-Z_]*\s*=" | \
            grep -v -E "//.*" | \
            grep -v -E "/\*.*\*/" | \
            grep -v -E "(Schema|Guard|Component|Provider|Context|Validator|Model|Type|Interface|Enum|Constant)" | \
            grep -v -E "\s+[A-Z][a-zA-Z]*\s*=\s*(\(|React\.|forwardRef|memo|lazy|createContext|styled)" | \
            grep -v -E "\.tsx:" | \
            head -20 || true)
          ...
          # Check function declarations - exclude PascalCase for component functions
          FUNCTION_ERRORS=$(grep -rn -E "function\s+[A-Z_][a-zA-Z0-9_]*\s*\(" src --include="*.ts" --include="*.tsx" --include="*.js" --include="*.jsx" | \
            grep -v -E "//.*" | \
            grep -v -E "(Guard|Component|Provider|Context|Validator|Model)" | \
            grep -v -E "\.tsx:" | \
            head -20 || true)
          ...
          # Check arrow function assignments - exclude PascalCase for components, schemas, etc.
          ARROW_FUNCTION_ERRORS=$(grep -rn -E "(const|let)\s+[A-Z_][a-zA-Z0-9_]*\s*=\s*\(" src --include="*.ts" --include="*.tsx" --include="*.js" --include="*.jsx" | \
            grep -v -E "(const|let)\s+[A-Z][A-Z_]*\s*=" | \
            grep -v -E "//.*" | \
            grep -v -E "(Schema|Guard|Component|Provider|Context|Validator|Model)" | \
            grep -v -E "\.tsx:" | \
            head -20 || true)
          ...
          if [ $ERROR_COUNT -gt 0 ]; then
            ...
            echo "React components, guards, schemas, and similar constructs should use PascalCase"
            exit 1
          fi

          echo "✅ All variables follow proper naming conventions"
